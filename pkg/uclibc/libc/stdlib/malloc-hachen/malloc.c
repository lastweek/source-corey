...
malloc_state_init(void)
{
    int r = segment_alloc(core_env->sh, sizeof(*__malloc_state), 0, 
			  (void **)&__malloc_state, SEGMAP_HEAP, 
			  "malloc-state", core_env->pid);
    if (r < 0) {
	cprintf("malloc_state_init: __malloc_state allocation failed: %s", 
		e2s(r));
	processor_halt();
    }
	
    __UCLIBC_MUTEX_INIT(x, PTHREAD_RECURSIVE_MUTEX_INITIALIZER_NP);
    memcpy(&__malloc_state->__malloc_lock, &x, 
	   sizeof(__malloc_state->__malloc_lock));
    ummap_alloc_init();
}
