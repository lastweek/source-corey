...
if [ $# -ne "7" ]
    ...
    echo " filesums-size : size of filesum request"
    exit 1
fi
...
while [ $server_index -lt $server_num ]
  ...
  while [ $client_it -lt $client_per_server ]
    ...
    ssh -i $ssh_key -l root $c "$httpc_command $s $port -c $threads -s $sockets -f $working_dir/httpc.conf -z $filesum_sz > $working_dir/\`hostname\`; scp -i $ssh_client_key $working_dir/\`hostname\` $results_ip:$results_path" &
