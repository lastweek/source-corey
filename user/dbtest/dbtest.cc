...
uint64_t cpufreq = 1900000000ULL;

struct sstate {
	volatile uint64_t result[MAX_CORE_NUM];
	volatile uint32_t channel[2];
};
...
int main(int argc, char * argv[])
{
	...
	struct sobj_ref * ummap_shref = NULL;
	...
	if (argc > 1) {
		...
		create_indexes_ = atoi(argv[1]);
		num_databases_ = atoi(argv[2]);
		num_selects_ = atoi(argv[3]);
		num_threads_ = atoi(argv[4]);
		num_rows_ = atoi(argv[5]);
		max_c2_val_ = atoi(argv[6]);
		do_global_ = atoi(argv[7]);
		do_poke_ = atoi(argv[8]);
		c3_str_length_ = atoi(argv[9]);
		shared_address_ = atoi(argv[10]);
	}

	// allocate the dynamic arrays
	dbnames_ = (char**) malloc (num_databases_ * sizeof (char*));
	c3_str_buf = (char*) malloc ((c3_str_length_+1) * sizeof (char));
	c3_str_buf[c3_str_length_] = '\0';
	...
	cprintf("---------------------\n");
	cprintf("create_indexes_=%u\n", create_indexes_);
	cprintf("num_databases_=%u\n", num_databases_);
	cprintf("num_selects_=%u\n", num_selects_);
	cprintf("num_threads_=%u\n", num_threads_);
	cprintf("num_rows_=%u\n", num_rows_);
	cprintf("max_c2_val_=%u\n", max_c2_val_);
	cprintf("%s\n", do_global_ ? "global" : "local");
	cprintf("%s\n", do_poke_ ? "poking" : "scanning");
	cprintf("c3_str_length_=%u\n", c3_str_length_);
	cprintf("shared_address_=%u\n", shared_address_);
	cprintf("---------------------\n");
	...
	if (shared_address_) {
		shared_state = (struct sstate *)malloc(sizeof(struct sstate));
	...
	for (uint32_t i = 1; i < num_threads_; i++) {
		...
		int64_t r = pforkv(i, ummap_shref ? PFORK_SHARE_HEAP : 0, ummap_shref, ummap_shref ? 1 : 0);
		if (r < 0)
